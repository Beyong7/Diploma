...
def plot_error_distances(all_contours, reference_contour_index, reference_slice_index, threshold=5):
    ...
    for slice_number in sorted(contours_by_slice.keys()):
        ...
        if dist_0 <= threshold and dist_0 / 10 < 3:
            distances_0.append(dist_0)
        if dist_90 <= threshold and dist_90 / 10 < 3:
            distances_90.append(dist_90)
        if dist_180 <= threshold and dist_180 / 10 < 3:
            distances_180.append(dist_180)
        if dist_last <= threshold and dist_last / 10 < 3:
...
def main(dicom_directory):
    ...
    if all_contours:
        ...
        draw_contour(image, all_contours[10])  # Рисуем первый контур
        ...
        plot_error_distances(all_contours, reference_contour_index=10, reference_slice_index=51,
                             threshold=50)  # 80-й файл имеет индекс 52


if __name__ == "__main__":
    dicom_directory = "/home/beyong/Coding/NIR/Phantom/Phantom/Phantom2/MRI2/MRI2"  # Укажите путь к вашим DICOM-файлам
